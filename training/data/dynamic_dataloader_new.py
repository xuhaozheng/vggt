...
def build_dynamic_dataloader(
    ...
    # sampler = RandomSampler(dataset_obj,aspect_ratio_range)
    # sampler = DynamicRandomSampler(dataset_obj, aspect_ratio_range, seed=seed)
...
class DynamicRandomSampler(RandomSampler):
    """
    Extends PyTorch's RandomSampler to include dynamic aspect_ratio and image_num
    parameters, which can be passed into the dataset's __getitem__ method.
    """
    def __init__(
        self,
        ...
        """
        Initializes the dynamic random sampler.
        ...
        """
        super().__init__(data_source, replacement=False)
