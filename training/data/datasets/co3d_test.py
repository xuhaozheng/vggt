...
class Co3dDataset(BaseDataset):
    def __init__(
        ...
        for c in category:
            for split_name in split_name_list:
                ...
                for i in range(10000):
                    data_entry = annotation[i]

                    sequence_name = data_entry['sequence_name']
                    if sequence_name not in self.data_store.keys():
                        self.data_store[sequence_name] = []
                    self.data_store[sequence_name].append(data_entry)
        ...
        print(f"{status}: Co3D Data length of training set: {len(self)}")
        print(f"{status}: Co3D Data size: {self.sequence_list_len}")
    ...
    def get_data(
        ...
        for anno in annos:
            ...
            R = np.array(viewpoint['R'])  # Rotation matrix (3x3)
            T = np.array(viewpoint['T']).reshape(3, 1)  # Translation vector (3x1)
            focal_length = viewpoint['focal_length']
            principal_point = viewpoint['principal_point']

            # extri_opencv, intri_opencv = co3d_annotation_to_opencv_pose(R,T,principal_point,focal_length,h,w)
            extri_opencv, intri_opencv = opencv_from_cameras_projection_numpy(R, T, principal_point, focal_length, h, w)
            print('intri_opencv',intri_opencv)
